...
function describeElement(element) {
  ...
  if (!element.dataset.plutomixUiId) {
    const all = Array.from(document.querySelectorAll(selectableSelector));
    const index = Math.max(1, all.indexOf(element) + 1);
    element.dataset.plutomixUiId = explicitId ? slug(explicitId) : `ui-${element.tagName.toLowerCase()}-${slug(label)}-${index}`;
  }
  return {
    id: element.dataset.plutomixUiId,
...
export default function PlutoMixReferenceBridge() {
  useEffect(() => {
    let enabled = false;
    ...
    style.textContent = `
      .plutomix-reference-hover {
        outline: 0 !important;
        border-radius: 10px !important;
        box-shadow: 0 0 0 3px rgba(37, 99, 235, 0.28), 0 0 0 7px rgba(20, 184, 166, 0.16) !important;
        animation: plutomix-reference-glow 1.1s ease-in-out infinite !important;
        cursor: crosshair !important;
      }
      .plutomix-reference-selected {
        outline: 0 !important;
        border-radius: 10px !important;
        box-shadow: 0 0 0 3px rgba(34, 211, 238, 0.32), 0 0 0 9px rgba(59, 130, 246, 0.16), 0 0 26px rgba(139, 92, 246, 0.26) !important;
        animation: plutomix-reference-selected-glow 1.25s ease-in-out infinite !important;
      }
      .plutomix-reference-external-highlight {
        outline: 0 !important;
        border-radius: 10px !important;
        ...
        animation: plutomix-reference-rainbow 1.15s linear infinite !important;
      }
      .plutomix-reference-mode * {
        cursor: crosshair !important;
      }
      @keyframes plutomix-reference-glow {
      ...
      @keyframes plutomix-reference-selected-glow {
      ...
      @keyframes plutomix-reference-rainbow {
    ...
    const clearHover = () => {
      hoverElement?.classList.remove("plutomix-reference-hover");
      hoverElement = null;
    };

    const clearExternalHighlight = () => {
      externallyHighlightedElement?.classList.remove("plutomix-reference-external-highlight");
    ...
    const setMode = (nextEnabled) => {
      enabled = nextEnabled;
      document.documentElement.classList.toggle("plutomix-reference-mode", enabled);
    ...
    const onMouseOver = (event) => {
      ...
      clearHover();
      hoverElement = target;
      hoverElement.classList.add("plutomix-reference-hover");
    };
    ...
    const onClick = (event) => {
      ...
      target.classList.add("plutomix-reference-selected");
      clearHover();
      const reference = describeElement(target);
      window.parent?.postMessage({ type: "plutomix-ui-reference-selected", reference }, "*");
    };

    const onKeyDown = (event) => {
      if (event.key === "Escape") {
        setMode(false);
        window.parent?.postMessage({ type: "plutomix-ui-reference-cancelled" }, "*");
      }
    };

    const onMessage = (event) => {
      if (event.data?.type === "plutomix-reference-mode") setMode(Boolean(event.data.enabled));
      if (event.data?.type === "plutomix-reference-highlight") {
        clearExternalHighlight();
        if (!event.data.active) return;
        externallyHighlightedElement = elementForReference(event.data.reference);
        externallyHighlightedElement?.classList.add("plutomix-reference-external-highlight");
      }
    };
    ...
    return () => {
      setMode(false);
      clearExternalHighlight();
      selectedElements.forEach((element) => element.classList.remove("plutomix-reference-selected"));
