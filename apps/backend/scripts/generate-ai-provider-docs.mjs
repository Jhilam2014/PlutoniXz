...
import { capabilityMatrix } from "../src/aiProviders/metadata.js";

const scriptDir = path.dirname(fileURLToPath(import.meta.url));
const outputPath = path.resolve(scriptDir, "../../../docs/ai-provider-profiles.md");
const yes = (value) => value ? "Yes" : "—";
const rows = capabilityMatrix().map((provider) => {
  const capabilities = new Set(provider.capabilities);
  return `| ${provider.name} | ${provider.minimumVersion || "Detect only"} | ${provider.authMethods.join(", ").replaceAll("_", " ")} | ${yes(capabilities.has("login"))} | ${yes(capabilities.has("logout"))} | ${yes(capabilities.has("multiple_profiles"))} | ${yes(capabilities.has("profile_switch"))} | ${yes(capabilities.has("token_refresh"))} | ${yes(capabilities.has("usage"))} | ${yes(capabilities.has("model_discovery"))} | ${yes(provider.gothamExecutionSupported)} |`;
}).join("\n");

const document = `# Gotham Chat AI provider profiles
...
Only Codex currently advertises the hardened Gotham workspace execution contract. Other connected accounts can be managed in the central panel but cannot be selected for a Gotham file-writing job until their execution/sandbox/event contracts are separately approved.

## Installation and login

Install each desired CLI in the backend runtime image or host process and optionally set its approved executable variable: \`CODEX_BIN\`, \`CLAUDE_BIN\`, \`COPILOT_BIN\`, \`CURSOR_AGENT_BIN\`, or \`EMERGENT_BIN\`. A configured path is accepted only when its basename matches the adapter's executable allowlist; browser input cannot choose an executable or add arguments.
