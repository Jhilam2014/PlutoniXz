...
function agentInstructionCandidates() {
  return [
    process.env.PLUTONIX_AGENT_INSTRUCTION_PATH,
    path.resolve(moduleDir, "../../../agents/generated/plutonix-fullstack-agent.agent.md"),
    path.resolve(process.cwd(), "agents/generated/plutonix-fullstack-agent.agent.md"),
    "/workspace/project/agents/generated/plutonix-fullstack-agent.agent.md"
  ].filter(Boolean);
}

function canonicalInstructionCandidates() {
  return [
    process.env.PLUTONIX_CANONICAL_POLICY_PATH,
...
async function readFirstExisting(paths) {
  ...
  throw new Error("PlutoniX Fullstack Agent instruction file is unavailable; global orchestration cannot start.");
}
...
export async function createPlutoniXOrchestrationEnvelope({ instruction, taskType = "Medium", project = null, structuredRequest }) {
  ...
  const parentWorkflowId = `plutonix_${crypto.randomUUID()}`;
  ...
  return {
    ...
    constraints: [
      ...
      "PlutoniX Fullstack Agent retains planning, sequencing, validation, retry, and completion authority.",
