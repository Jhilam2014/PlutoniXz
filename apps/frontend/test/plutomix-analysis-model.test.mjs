...
} from "../src/plutomixAnalysisModel.js";
...
test("frontend origin never treats runtime status as creation provenance", () => {
  ...
  assert.equal(applicationOrigin({ status: "stopped", provenance: { origin: "plutomix_created" } }).kind, "plutomix_created");
  assert.deepEqual(applicationOrigin({ status: "stopped", createdAt: "2026-01-01T00:00:00.000Z", productDecision: { productShape: "web_app" } }), {
    kind: "plutomix_created",
    label: "PlutoMix-created",
...
test("current source maps hide stale source projections without deleting governed ledger history", () => {
  const summary = applicationDecisionSummary({
    ...
    project: { provenance: { origin: "plutomix_created" } }
  });
...
test("application summaries retain source-report branches, evidence ids, and ledger precedence", () => {
  ...
  const createdProject = { provenance: { origin: "plutomix_created" } };
...
test("the mounted PlutoMix analysis workspace replaces the retired canvases with document-native flows", () => {
  const app = fs.readFileSync(new URL("../src/App.jsx", import.meta.url), "utf8");
  const workspace = fs.readFileSync(new URL("../src/PlutoMixAnalysisWorkspace.jsx", import.meta.url), "utf8");
  const server = fs.readFileSync(new URL("../../backend/src/server.js", import.meta.url), "utf8");

  assert.ok(app.includes("<PlutoMixAnalysisWorkspace"));
...
test("portfolio analysis exposes a governed place to record account and client sharing context", () => {
  ...
  assert.ok(workspace.includes('className="plutomix-information-sharing-table"'));
