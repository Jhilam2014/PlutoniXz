...
const APPLICATION_ORIGINS = new Set(["plutomix_created", "imported", "unknown_legacy"]);
...
export function applicationOrigin(value = {}) {
  ...
  if (explicitOrigin === "unknown_legacy" && Object.keys(legacyProductDecision).length) explicitOrigin = "plutomix_created";
  const provenance = candidates.find((candidate) => text(candidate.origin).toLowerCase() === explicitOrigin) || {};
  return {
    kind: explicitOrigin,
    label: explicitOrigin === "plutomix_created"
      ? "PlutoMix-created"
    ...
    source: text(provenance.source) || (Object.keys(legacyProductDecision).length ? "legacy_plutomix_product_decision" : explicitOrigin === "unknown_legacy" ? "provenance_not_recorded" : "project_record"),
...
export function buildBrainHierarchy({ portfolio = null, directory = [] } = {}) {
  ...
  return {
    enterprise: {
      label: text(enterpriseBrain.label || enterpriseBrain.name) || "PlutoMix Enterprise Brain",
...
export function sourceDecisionBranches({ architectureAnalysisReport = null, project = null } = {}) {
  ...
  if (origin.kind !== "plutomix_created") {
    for (const functionality of functionalities) {
