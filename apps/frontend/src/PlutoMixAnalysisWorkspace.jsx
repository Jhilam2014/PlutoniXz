...
function PortfolioMessage({ state, error, children }) {
  if (state === "loading") return <p className="plutomix-analysis-state" role="status">Loading portfolio analysis…</p>;
  if (state === "error") return <p className="plutomix-analysis-state plutomix-analysis-state-error" role="alert">{error}</p>;
  return children || null;
}

function ApplicationDirectory({ applications, selectedProjectId, search, onSearch, onSelectProject }) {
  return (
    <aside className="plutomix-analysis-directory" aria-label="Application directory">
      <header className="plutomix-analysis-directory-header">
        <div>
          <span className="plutomix-analysis-eyebrow">Enterprise portfolio</span>
          <h2>Applications</h2>
        </div>
        <span className="plutomix-analysis-directory-count">{countLabel(applications.length, "application")}</span>
      </header>
      <label className="plutomix-analysis-search-label">
        <span>Find application</span>
        <input
          className="plutomix-analysis-search"
          type="search"
          value={search}
      ...
      {applications.length ? (
        <ol className="plutomix-analysis-directory-list">
          {applications.map((application) => (
            <li key={application.id}>
              <button
                className="plutomix-analysis-directory-item"
                type="button"
                aria-current={application.id === selectedProjectId ? "page" : undefined}
                onClick={() => onSelectProject(application.project)}
              >
                <span className="plutomix-analysis-directory-item-copy">
                  <strong>{application.name}</strong>
                  <small>{application.summary || "No portfolio summary is recorded."}</small>
                </span>
                <span className="plutomix-analysis-directory-item-meta">
                  <span className="plutomix-analysis-state-badge">{application.status}</span>
        ...
        <p className="plutomix-analysis-empty">No managed applications match this search.</p>
      )}
    </aside>
  );
}
...
function EnterpriseBrainHierarchy({ hierarchy, selectedProjectId }) {
  ...
  return (
    <section className="plutomix-analysis-brain-hierarchy" aria-labelledby="plutomix-analysis-brain-heading">
      <header>
        <span className="plutomix-analysis-eyebrow">Intelligence boundary</span>
        <h3 id="plutomix-analysis-brain-heading">Enterprise Brain and App BrainX</h3>
      </header>
      <ol className="plutomix-analysis-brain-list">
        <li className="plutomix-analysis-brain-enterprise">
        ...
        {selected ? (
          <li className="plutomix-analysis-brain-application">
...
function EnterpriseAssignmentEditor({ project, onProjectUpdated }) {
  ...
  return (
    <section className="plutomix-analysis-enterprise-editor" aria-labelledby="plutomix-analysis-enterprise-heading">
      <header>
        <div>
          <span className="plutomix-analysis-eyebrow">Portfolio classification</span>
          <h3 id="plutomix-analysis-enterprise-heading">Enterprise assignment</h3>
        </div>
      </header>
      <form className="plutomix-analysis-enterprise-form" onSubmit={saveEnterprise}>
        <label>
          ...
          <input
            ...
            aria-describedby="plutomix-analysis-enterprise-editor-help"
          />
        </label>
        <label>
        ...
        <p id="plutomix-analysis-enterprise-editor-help" className={`plutomix-analysis-enterprise-editor-help ${draft.isInvalid ? "is-error" : ""}`}>{editorHelp}</p>
        <button className="plutomix-analysis-enterprise-save" type="submit" disabled={!canSave} aria-busy={state === "saving"}>
          {actionLabel}
        </button>
      </form>
      {message ? <p className={`plutomix-analysis-enterprise-message ${state === "error" ? "plutomix-analysis-enterprise-message-error" : ""}`} role={state === "error" ? "alert" : "status"}>{message}</p> : null}
    </section>
  );
}
...
function EnterprisePortfolioAssignmentPanel({ applications, onProjectsUpdated }) {
  ...
  return (
    <section className="plutomix-analysis-enterprise-assignment-panel" aria-labelledby="plutomix-analysis-enterprise-assignment-panel-heading">
      <header>
        <div>
          <span className="plutomix-analysis-eyebrow">Portfolio management</span>
          <h3 id="plutomix-analysis-enterprise-assignment-panel-heading">Add applications to an enterprise</h3>
        ...
        <span className="plutomix-analysis-directory-count" role="status" aria-live="polite">{countLabel(selectedIds.length, "selected application")}</span>
      </header>
      <form className="plutomix-analysis-enterprise-assignment-grid" onSubmit={applyAssignment}>
        <fieldset className="plutomix-analysis-enterprise-application-picker">
          ...
          <div className="plutomix-analysis-enterprise-selection-actions">
          ...
          <div className="plutomix-analysis-enterprise-application-list" role="group" aria-label="Managed applications">
            ...
            )) : <p className="plutomix-analysis-empty">No applications match this search.</p>}
          </div>
        </fieldset>
        <fieldset className="plutomix-analysis-enterprise-target-picker">
          ...
          {isCreating ? (
            <div className="plutomix-analysis-enterprise-form">
              <label>
                <span>Enterprise name</span>
                <input
                  id="plutomix-analysis-new-enterprise-name"
                  value={enterpriseName}
                  onChange={(event) => {
                  ...
                  aria-describedby="plutomix-analysis-enterprise-validation"
                />
              </label>
              <label>
                <span>Enterprise ID</span>
                <input
                  id="plutomix-analysis-new-enterprise-id"
                  value={enterpriseId}
                  onChange={(event) => {
          ...
          {enterpriseChoice && !isCreating && !isRemoving ? <div className="plutomix-analysis-enterprise-target-summary"><strong>{enterpriseName}</strong><small>{enterpriseId} · existing enterprise</small></div> : null}
          {isCreating ? <p id="plutomix-analysis-enterprise-validation" className="plutomix-analysis-enterprise-help">This records the first application membership under a new enterprise label; it does not create a sharing agreement or Enterprise Brain. Enter a name and a lowercase ID of at least two characters. IDs may contain numbers and hyphens.</p> : null}
          <div className="plutomix-analysis-enterprise-review">
          ...
          {assignmentPlan.length ? (
            <ul className="plutomix-analysis-enterprise-change-list" aria-label="Enterprise assignment changes">
          ...
          {confirmationRequired ? (
            <label className="plutomix-analysis-enterprise-confirmation">
          ...
          <p className="plutomix-analysis-enterprise-batch-note">Updates are applied to each application individually. Completed assignments remain saved if another application fails.</p>
          <button className="plutomix-analysis-enterprise-save" type="submit" disabled={!canApply}>
            {state === "saving" ? `Updating ${progress.complete} of ${progress.total}…` : actionLabel}
          </button>
          {results.length ? (
            <div className={`plutomix-analysis-enterprise-results ${results.some((result) => result.status === "failed") ? "has-errors" : ""}`}>
              <p className="plutomix-analysis-enterprise-result-summary" role={results.some((result) => result.status === "failed") ? "alert" : "status"} aria-atomic="true">{state === "saved" ? "Enterprise assignments updated." : "Some assignments need attention."}</p>
          ...
          ) : state === "saving" ? <p className="plutomix-analysis-enterprise-message" role="status">Updating application {progress.complete + 1} of {progress.total}.</p> : null}
        </fieldset>
      </form>
    </section>
  );
...
function PortfolioRelations({ relations, applications, selectedProjectId = "", title = "Portfolio relations" }) {
  ...
  return (
    <section className="plutomix-analysis-relations" aria-labelledby="plutomix-analysis-relations-heading">
      <header>
        <span className="plutomix-analysis-eyebrow">Recorded connections</span>
        <h3 id="plutomix-analysis-relations-heading">{title}</h3>
      </header>
      {visibleRelations.length ? (
        <div className="plutomix-analysis-relations-table-wrap">
          <table>
            <thead>
            ...
            <tbody>
              {visibleRelations.map((relation) => (
                <tr key={relation.id}>
                  ...
                  <td><span className={`plutomix-analysis-relation-kind plutomix-analysis-relation-kind-${relation.kind}`}>{relation.kindLabel}</span></td>
      ...
      ) : <p className="plutomix-analysis-empty">No application relationship has been recorded for this scope. PlutoMix does not infer portfolio dependencies from shared names, tags, or agents.</p>}
    </section>
  );
}
...
function PortfolioRelationFlow({ relations, applications, selectedProjectId = "", title = "Relation flow" }) {
  ...
  return (
    <section className="plutomix-analysis-relation-flow" aria-labelledby="plutomix-analysis-relation-flow-heading">
      <header>
        <span className="plutomix-analysis-eyebrow">Recorded relation flow</span>
        <h3 id="plutomix-analysis-relation-flow-heading">{title}</h3>
        <p>Each row is an explicit left-to-right record. Causal dependencies and authorized sharing rights are separate.</p>
      </header>
      {groups.length ? groups.map((group) => (
        <section className="plutomix-analysis-relation-flow-group" key={group.kind} aria-labelledby={`plutomix-analysis-relation-flow-${group.kind}`}>
          <h4 id={`plutomix-analysis-relation-flow-${group.kind}`}>{group.label}</h4>
          <ol>
            {group.relations.map((relation) => (
              <li key={relation.id}>
                <article className="plutomix-analysis-relation-endpoint"><small>Source application</small><strong>{applicationsById.get(relation.sourceProjectId) || relation.sourceProjectId}</strong></article>
                <div className="plutomix-analysis-relation-arrow" aria-hidden="true">→</div>
                <div className="plutomix-analysis-relation-label"><span className={`plutomix-analysis-relation-kind plutomix-analysis-relation-kind-${relation.kind}`}>{relation.kindLabel}</span><strong>{relation.label.replaceAll("_", " ")}</strong><small>{relation.kind === "authorized_information_sharing" ? countLabel(relation.agreementCount, "active agreement") : countLabel(relation.evidenceCount, "recorded reference")}</small></div>
                <div className="plutomix-analysis-relation-arrow" aria-hidden="true">→</div>
                <article className="plutomix-analysis-relation-endpoint"><small>Target application</small><strong>{applicationsById.get(relation.targetProjectId) || relation.targetProjectId}</strong></article>
              </li>
            ))}
          </ol>
        </section>
      )) : <p className="plutomix-analysis-empty">No cross-application relation is recorded for this scope, so no flow is shown.</p>}
    </section>
  );
}
...
function BranchDetailPanel({ checkpoint, reconsiderations }) {
  if (!checkpoint) {
    return <aside className="plutomix-analysis-branch-detail" aria-label="Decision detail"><p className="plutomix-analysis-empty">Choose a recorded outcome to inspect its rationale, constraints, evidence, and reconsideration signals.</p></aside>;
  }
  return (
    <aside className="plutomix-analysis-branch-detail" aria-labelledby="plutomix-analysis-branch-detail-heading">
      <header>
        <span className="plutomix-analysis-eyebrow">Checkpoint outcome</span>
        <span className={`plutomix-analysis-decision-state plutomix-analysis-decision-state-${checkpoint.state}`}>{decisionStateLabel(checkpoint.state, checkpoint.recordClassification)}</span>
      </header>
      <h3 id="plutomix-analysis-branch-detail-heading">{checkpoint.label}</h3>
      <dl className="plutomix-analysis-branch-detail-list">
...
function DecisionBranchCard({ branch, selected, relationship, onSelect, onNavigate }) {
  return (
    <button
      className={`plutomix-analysis-lineage-card plutomix-analysis-lineage-card-${branch.state} ${selected ? "is-selected" : ""}`}
      type="button"
      data-decision-branch-id={branch.id}
      aria-pressed={selected}
      aria-controls="plutomix-analysis-branch-detail-heading"
      onClick={() => onSelect(branch.id)}
      onKeyDown={(event) => onNavigate?.(event, branch.id)}
    >
      <span className={`plutomix-analysis-decision-state plutomix-analysis-decision-state-${branch.state}`}>{decisionStateLabel(branch.state, branch.recordClassification)}</span>
...
function DecisionAgentNodes({ agents = [], functionalityLabel = "this functionality", compact = false }) {
  ...
  return (
    <section className={`plutomix-analysis-decision-agent-nodes ${compact ? "is-compact" : ""}`.trim()} aria-label={`Direct agent nodes for ${functionalityLabel}`}>
      <header><span className="plutomix-analysis-eyebrow">Direct agent nodes</span><small>{agents.length === 1 ? "1 explicit association" : `${agents.length} explicit associations`}</small></header>
      <ol>
        {agents.map((agent) => (
...
function DecisionPathBoard({ summary, reconsiderations, agentNodesByFunctionality = null }) {
  ...
  const navigateBranch = (event, currentBranchId) => {
    if (event.key === "Escape") {
      event.preventDefault();
      document.getElementById(view === "recorded" ? "plutomix-analysis-lineage-root-select" : "plutomix-analysis-source-checkpoint-select")?.focus();
      return;
    }
  ...
  if (!sourceCheckpoints.length && !recordedLineages.length) {
    return <section className="plutomix-analysis-decision-board" aria-label="Application decisions"><p className="plutomix-analysis-empty">No source-backed functionality or recorded decision lineage is available. Analyze this application to create an observed decision map; PlutoMix will not invent historical choices.</p></section>;
  }
  ...
  return (
    <section className="plutomix-analysis-decision-board plutomix-analysis-lineage-board" aria-labelledby="plutomix-analysis-decision-heading">
      <header>
        <div>
          <span className="plutomix-analysis-eyebrow">Application decision analysis</span>
          <h3 id="plutomix-analysis-decision-heading">Decision lineage</h3>
          <p>{origin.kind === "plutomix_created"
        ...
        <div className="plutomix-analysis-lineage-heading-meta">
          <span className={`plutomix-analysis-origin-badge is-${origin.kind}`}>{origin.label}</span>
          <small>{countLabel(recordedLineages.length, "recorded root")} · {countLabel(sourceCheckpoints.length, "source checkpoint")}</small>
        </div>
      </header>
      <div className="plutomix-analysis-lineage-legend" aria-label="Decision graph key">
      ...
      {recordedLineages.length && sourceCheckpoints.length ? (
        <nav className="plutomix-analysis-lineage-view-switcher" aria-label="Decision graph source">
      ...
      {view === "recorded" && activeLineage ? (
        <>
          <div className="plutomix-analysis-lineage-toolbar">
            <label className="plutomix-analysis-functionality-picker">
              <span>Decision root</span>
              <select id="plutomix-analysis-lineage-root-select" value={activeLineage.id} onChange={(event) => { setLineageId(event.target.value); setBranchId(""); }}>
                {recordedLineages.map((lineage) => <option key={lineage.id} value={lineage.id}>{lineage.label}</option>)}
              </select>
            </label>
            <div className="plutomix-analysis-lineage-order-note">
          ...
          <ol className="plutomix-analysis-root-index" aria-label="Recorded decision roots">
          ...
          {lineageAnticipatedChoices.length ? (
            <section className="plutomix-analysis-lineage-anticipated-options" aria-label={`Anticipated options for ${activeLineage?.label || "this lineage"}`}>
              <header><div><span className="plutomix-analysis-eyebrow">Related anticipated option nodes</span><strong>Source-derived options outside the recorded lineage</strong></div><small>Not historical edges</small></header>
          ...
          <div className="plutomix-analysis-lineage-layout">
            <div className="plutomix-analysis-lineage-scroll" role="group" aria-label={`${activeLineage.label} recorded branch lineage`}>
              <div className="plutomix-analysis-lineage-flow">
                {ranks.map(({ depth, nodes, incomingEdges, parentGroups }, rankIndex) => (
                  <section className="plutomix-analysis-lineage-rank" key={depth} aria-label={depth === 0 ? "Decision root" : `Recorded branch depth ${depth}`}>
                    {rankIndex ? <div className="plutomix-analysis-lineage-connector" aria-hidden="true"><span>→</span></div> : null}
                    <div className="plutomix-analysis-lineage-rank-copy"><span>{depth === 0 ? "Root" : `Branch ${depth}`}</span><small>{countLabel(nodes.length, "record")} · {countLabel(incomingEdges.length, "stored link")}</small></div>
                    <div className="plutomix-analysis-lineage-rank-cards">
                      {parentGroups.map(([parentId, children]) => {
                        ...
                        return (
                          <section className={`plutomix-analysis-lineage-parent-group ${parent ? "has-parent" : ""}`} key={parentId} aria-label={parent ? `Branches from ${parent.label}` : "Recorded root branch"}>
                            {parent ? <div className="plutomix-analysis-lineage-parent-label"><span aria-hidden="true">↳</span><small>Branches from</small><strong>{parent.label}</strong></div> : null}
              ...
              {summary.unlinkedBranches?.length ? <section className="plutomix-analysis-unlinked-branches"><strong>Unlinked recorded branches</strong><p>These records have no safe lineage connector.</p><ul>{summary.unlinkedBranches.map((branch) => <li key={branch.id}><button type="button" onClick={() => setBranchId(branch.id)}>{branch.label}<small>{branch.lineageIssue}</small></button></li>)}</ul></section> : null}
            </div>
      ...
      {view === "source" && activeCheckpoint ? (
        <>
          <div className="plutomix-analysis-lineage-toolbar">
            <div className="plutomix-analysis-checkpoint-navigation">
              ...
              <label className="plutomix-analysis-functionality-picker">
                ...
                <select id="plutomix-analysis-source-checkpoint-select" value={activeCheckpoint.id} onChange={(event) => { setFunctionalityId(event.target.value); setBranchId(""); }}>
          ...
          <ol className="plutomix-analysis-root-index" aria-label="Application source checkpoints">
          ...
          <div className="plutomix-analysis-lineage-layout">
            <div className="plutomix-analysis-source-choice-scroll" role="group" aria-label={`${activeCheckpoint.label} source-based choice map`}>
              <div className={`plutomix-analysis-source-choice-flow ${sourceAgents.length ? "has-agent-nodes" : ""}`.trim()}>
                <section className="plutomix-analysis-source-root-card">
                  <span className="plutomix-analysis-eyebrow">Application root</span>
                  <strong>{origin.label}</strong>
                  <small>{origin.kind === "imported" ? "External source baseline" : origin.kind === "plutomix_created" ? "PlutoMix build context" : "Origin unavailable"}</small>
                </section>
                <div className="plutomix-analysis-source-choice-arrow" aria-hidden="true">→</div>
                <section className="plutomix-analysis-source-checkpoint-card">
                  <span className="plutomix-analysis-eyebrow">Checkpoint {activeCheckpoint.displayOrder}</span>
...
function SharingPolicyNotice({ summary }) {
  ...
  return (
    <section className={`plutomix-analysis-sharing-policy plutomix-analysis-sharing-policy-${summary.agreementStatus}`} aria-label="Information-sharing policy status">
      <div><span className="plutomix-analysis-eyebrow">Information-sharing policy</span><strong>{label}</strong></div>
      <p>{copy}</p>
    </section>
  );
}
...
function InformationSharingPolicyEditor({ applications, policies = [], onSaved }) {
  ...
  return (
    <section className="plutomix-information-sharing" aria-labelledby="plutomix-information-sharing-heading">
      <header>
        <div><span className="plutomix-analysis-eyebrow">Governed context exchange</span><h3 id="plutomix-information-sharing-heading">Information-sharing policies</h3></div>
      ...
      <form onSubmit={savePolicy} className="plutomix-information-sharing-form">
        <div className="plutomix-information-sharing-table-wrap">
          <table className="plutomix-information-sharing-table">
      ...
      {policies.length ? <ol className="plutomix-information-sharing-list">{policies.map((policy) => {
      ...
      })}</ol> : <p className="plutomix-analysis-empty">No information-sharing policy is recorded. Cross-application context remains unavailable.</p>}
    </section>
  );
}
...
function PortfolioMode({ applications, assignmentApplications = applications, hierarchy, relations, portfolioSummary, informationSharingPolicies, onOpenApplication, onProjectsUpdated, onPolicySaved }) {
  ...
  return (
    <section className="plutomix-analysis-portfolio" aria-labelledby="plutomix-analysis-portfolio-heading">
      <header className="plutomix-analysis-content-header">
        <div>
          <span className="plutomix-analysis-eyebrow">Portfolio view</span>
          <h1 id="plutomix-analysis-portfolio-heading">Application portfolio</h1>
      ...
      <section className="plutomix-analysis-summary-strip" aria-label="Portfolio summary">
...
function ApplicationMode({ project, applications, hierarchy, relations, decisionState, remoteState, remoteError, remotePagination, remoteGraphEventCoverage, sourceMapState, sourceMapError, analysisSource, architectureAnalysisReport, architectureAnalysisError, onAnalyzeArchitecture, analyzingArchitecture, onProjectUpdated }) {
  ...
  if (!project?.id) {
    return <section className="plutomix-analysis-application" aria-label="Application analysis"><p className="plutomix-analysis-empty">Choose an application from the directory to inspect its decision analysis.</p></section>;
  }
  return (
    <section className="plutomix-analysis-application" aria-labelledby="plutomix-analysis-application-heading">
      <header className="plutomix-analysis-content-header">
        <div>
          <span className="plutomix-analysis-eyebrow">Application decisions</span>
          <h1 id="plutomix-analysis-application-heading">{project.name}</h1>
          <p>Project-scoped decision evidence. Current implementation and confirmed selection remain separate states.</p>
        </div>
        <div className="plutomix-analysis-application-actions">
      ...
      {architectureAnalysisError ? <p className="plutomix-analysis-state plutomix-analysis-state-error" role="alert">Architecture analysis: {architectureAnalysisError}</p> : null}
      {sourceMapState === "loading" ? <p className="plutomix-analysis-state" role="status">Reading the bounded source decision map…</p> : null}
      {sourceMapState === "error" ? <p className="plutomix-analysis-state plutomix-analysis-state-error" role="alert">Source decision map: {sourceMapError}</p> : null}
      {remoteState === "loading" ? <p className="plutomix-analysis-state" role="status">Loading recorded decision continuity…</p> : null}
      {remoteState === "error" ? <p className="plutomix-analysis-state plutomix-analysis-state-error" role="alert">{remoteError}{decisionState.reportBranchCount ? " Showing source-analysis records from the latest scoped report." : ""}</p> : null}
      {remoteState === "partial" ? <p className="plutomix-analysis-state plutomix-analysis-state-error" role="alert">Recorded decision continuity is incomplete. {remoteError || "Some recorded branch pages or supporting records could not be loaded."} {decisionContinuityCoverageLabel(remotePagination)}</p> : null}
      {remoteState === "truncated" ? <p className="plutomix-analysis-state plutomix-analysis-state-error" role="alert">Recorded decision continuity is truncated and must not be treated as complete. {remoteError || "The browser stopped at a safe pagination boundary."} {decisionContinuityCoverageLabel(remotePagination)}</p> : null}
      {remoteGraphEventCoverage === "unconfirmed" ? <p className="plutomix-analysis-state" role="status">Recorded event annotations may be incomplete: the graph API does not publish event pagination or total event coverage. Branch lineage uses the loaded branch ledger, not the event overlay.</p> : null}
      <section className="plutomix-analysis-summary-strip" aria-label="Application decision summary">
      ...
      {analysisSource ? <p className="plutomix-analysis-analysis-source">{analysisSource === "stale_recorded_source_analysis"
...
export default function PlutoMixAnalysisWorkspace({
  projects = [],
  ...
  return (
    <section className="plutomix-analysis-workspace" aria-label="PlutoMix enterprise portfolio and application decision analysis">
      <header className="plutomix-analysis-workspace-header">
        <div>
          <span className="plutomix-analysis-eyebrow">PlutoMix analysis</span>
          <strong>Enterprise portfolio and application decisions</strong>
        </div>
        <nav className="plutomix-analysis-mode-switcher" aria-label="Analysis mode">
      ...
      <div className="plutomix-analysis-layout">
        ...
        />
        <div className="plutomix-analysis-content">
