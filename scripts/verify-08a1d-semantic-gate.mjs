...
async function main() {
  ...
  if (process.argv.includes('--require-run') && expected !== 'ELIGIBLE_TO_RUN') fail('Full 08A1D rerun is prohibited until R3 has zero unresolved classes.');
  process.stdout.write(`08A1D semantic gate: ${expected}; full rerun ${expected === 'ELIGIBLE_TO_RUN' ? 'eligible' : 'not run'}.\n`);
}

main().catch((error) => { process.stderr.write(`08A1D semantic gate validation failed: ${error.message}\n`); process.exitCode = 1; });
