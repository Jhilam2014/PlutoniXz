# PlutoniX Enterprise And Application Analysis Workshop
...
- PlutoniX Enterprise Brain card with agreement boundary explanation
...
- `plutonix_created` means the project creation workflow recorded PlutoniX as its origin.
...
Starting, stopping, or rebuilding an application never changes its origin. Legacy `running` or `stopped` status is not treated as proof that PlutoniX created the app.
...
- PlutoniX-created, imported, and unknown-legacy origins are visibly distinguishable and never inferred from runtime state.
- A PlutoniX-created application can plot recorded selected/deferred/rejected lineage when those ledger records exist.
