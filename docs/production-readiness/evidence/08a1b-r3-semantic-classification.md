# 08A1B-R3 semantic classification

## Outcome

BLOCKED — one or more classes remain semantically unresolved; no provider or authority action is eligible for those classes.

| Measure | Count |
| --- | ---: |
| R2 equivalence classes | 1068 |
| Deterministic non-secret | 1 |
| Positive secret candidate | 0 |
| Semantically unresolved | 1067 |
| Active 08A1C external actions | 0 |

## Exact unresolved repository requirements

| Missing predicate | Classes |
| --- | ---: |
| AUTHENTICATION_OR_PRIVILEGED_CONSUMPTION_TRACE_REQUIRED | 10 |
| ENVIRONMENT_CANDIDATE_VARIABLE_UNRESOLVED | 1 |
| ENVIRONMENT_SOURCE_CONTEXT_REQUIRES_CONTROLLED_ANALYSIS | 2 |
| ENVIRONMENT_VALUE_FAILED_APPROVED_STRICT_PARSER | 1 |
| ENVIRONMENT_VARIABLE_HAS_NO_APPROVED_AUTHENTICATION_CONSUMER | 1 |
| EXACT_SOURCE_SCHEMA_PARSER_AND_CONSUMER_SEMANTICS_REQUIRED | 1057 |
| NO_APPROVED_PRODUCER_SCHEMA_CONSUMER_CONTRACT | 1053 |
| NON_FILE_OR_ARCHIVE_LOCATION_REQUIRES_SOURCE_LINEAGE | 8 |
| ORIGINAL_SOURCE_OR_HISTORY_CONTEXT_REQUIRES_BOUNDED_RETRIEVAL | 11 |
| SOURCE_IS_NOT_A_VALIDATED_STRUCTURED_RECORD | 1 |
| SOURCE_SCHEMA_OR_DETERMINISTIC_PRODUCER_CONTRACT_REQUIRED | 1066 |
