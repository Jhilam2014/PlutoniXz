...
npm run secret:reconcile:08a1b:semantic-publication:test
npm run secret:reconcile:08a1b:atomic
npm run secret:reconcile:08a1b:atomic:verify
npm run secret:reconcile:08a1c:semantic:verify
...
`npm run secret:reconcile:08a1b:atomic` is the recovery path for a frozen R2 snapshot. It replays only the paths named in the frozen, structurally redacted reports; reachable-history paths are pinned to that snapshot's commit boundary. Each scope has one 15-minute aggregate deadline across all replay targets. R2 equality and R3 semantic classification run in the same memory-only process, so no second full replay is required. The runner stages replacements, validates exact R2/R3 membership plus current R4/08A1D status, then promotes the staged set with rollback protection. It writes no candidate values, tags, hashes, or `.env` values. Controlled `.env` context can map a candidate only to one variable name and an approved local authentication consumer; ambiguous or unapproved values remain unresolved.

R3 Path A includes only explicitly proven non-authentication contracts. In addition to committed synthetic fixtures, the current contracts recognize the self-improvement `patternKey` and tool-plan `normalizedKey` only when their exact 24-hex stable-hash grammar, pinned producer, validated record field, and local consumer are all present. A generic 24-hex value, a different field, a missing producer, or a missing consumer remains unresolved. The semantic repair tests cover strict positive evidence, missing or ambiguous consumers, malformed values, raw-buffer clearing on success/failure/timeout paths, staged-publication rollback, and empty R4/08A1D queues while any class remains unresolved.

The former R4 external package is now `NON_ACTIONABLE_PENDING_SEMANTIC_TRIAGE`. Its historical requests are retained in the `.r4-audit.sanitized.json` files for audit but are not current pending authority/provider work. The only current package state is `08a1c-external-r4/current-semantic-triage-status.sanitized.json`, whose counts are derived from the validated current R3 classification. A full 08A1D rerun is allowed only after the R3 semantic gate passes with zero unresolved classes; otherwise 08A1D is explicitly `NOT RUN`.
