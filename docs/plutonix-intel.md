...
PlutoniX Intel is a profile-driven workflow layered over the normal project workflow. With Intel off, the existing direct workflow is unchanged. With Intel on, Product Shape, the instruction, and existing-project metadata select a profile before any writer starts.
