...
Gotham Builder instruction rows expose an **Export complete log (.txt)** action after the instruction has a durable sequence ID. The backend rebuilds the export from the authoritative project instruction plus retained workflow events, redacts credential material without shortening individual retained fields, saves the exact downloadable text and SHA-256 digest, and returns a tenant/project-scoped download reference. Exports have a 4 MiB safety ceiling and carry an explicit truncation marker if that ceiling is reached. Production uses `gotham_instruction_log_exports` from migration `016_gotham_instruction_log_exports.sql`; `scripts/deploy-plutomix.sh` applies the locked migration set after building and before starting the API. A manual rollout may instead run `npm --prefix apps/backend run instruction-log-exports:migrate` (or the full decision-continuity migration) before deploying the backend. Local development uses the atomic, gitignored `runtime/gotham-instruction-log-exports.json` store by default; `GOTHAM_INSTRUCTION_LOG_EXPORT_PATH` may relocate it.
